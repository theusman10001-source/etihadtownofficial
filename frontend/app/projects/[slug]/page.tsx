        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gradient-to-r from-primary to-primary-light text-left text-white">
              ...
              {hasBlock && (
                <th className="px-4 py-4 font-semibold">Block / Plan</th>
              )}
              ...
              <th className="px-4 py-4 text-right font-semibold">
                Monthly ({rows[0]?.installments})
              </th>
              {hasBalloon && (
                <th className="px-4 py-4 text-right font-semibold">Balloon</th>
              )}
              {hasAnnual && (
                <th className="px-4 py-4 text-right font-semibold">Annual</th>
              )}
              {hasBallot && (
                <th className="px-4 py-4 text-right font-semibold">Ballot</th>
              )}
              {hasPossession && (
                <th className="px-4 py-4 text-right font-semibold">
                  Possession
...
function PriceRow({
  ...
  c: string;
  last: boolean;
  show: [boolean, boolean, boolean, boolean, boolean, boolean, boolean];
}) {
  const [
    hasBlock,
    hasStatus,
  ...
  return (
    <tr
      className={`bg-white ${last ? "" : "border-b border-neutral-100"} hover:bg-soft/60 transition-colors`}
    >
      <td className="px-4 py-3.5 font-semibold text-primary">{row.size}</td>
      {hasBlock && (
        <td className="px-4 py-3.5 text-xs text-neutral-500">{row.block}</td>
      )}
      {hasStatus && (
        <td className="px-4 py-3.5">
          <span
            className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${rowStatusStyle[row.status ?? ""] ?? "bg-accent/10 text-accent-dark"}`}
          >
            {row.status}
          </span>
        </td>
      ...
      {hasBalloon && (
        <td className="px-4 py-3.5 text-right text-neutral-600">
          {row.balloons && row.balloonAmount
            ? `${row.balloons} × ${formatPrice(row.balloonAmount, c)}`
            : "—"}
        </td>
      )}
      {hasAnnual && (
        <td className="px-4 py-3.5 text-right text-neutral-600">
          {row.annualPayments && row.annualAmount
            ? `${row.annualPayments} × ${formatPrice(row.annualAmount, c)}`
            : "—"}
        </td>
      )}
      {hasBallot && (
    ...
    <div className="mt-12">
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
        {features.map((f) => (
          <div
            key={f}
            className="flex items-center gap-3 rounded-xl border border-neutral-100 bg-white px-4 py-3 text-sm text-neutral-600 transition-colors hover:border-accent/30"
          >
            ...
            <div className="lg:col-span-2">
              <Reveal delay={0.05}>
                ...
                {project.longDescription.map((para, i) => (
                  <p
                    key={i}
                    className="mt-4 text-sm leading-[1.8] text-neutral-500"
                  >
                    {para}
                  </p>
                ))}
                    ...
                    <div className="mt-12">
                      ...
                      <ul className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        {project.locationHighlights.map((h) => (
                          <li
                            key={h}
                            className="flex items-start gap-2.5 rounded-xl border border-neutral-100 bg-white px-4 py-3 text-sm text-neutral-600"
                          >
                            <svg
                ...
                <div className="overflow-hidden rounded-3xl border border-white/10 bg-primary shadow-[0_30px_80px_rgba(7,30,69,0.35)]">
                  ...
                  <div className="p-7">
                    <p className="text-sm leading-relaxed text-white/75">
                      Our team can share current availability, exact pricing,
                      and guide you through the booking process — all in a
                      single conversation.
                    </p>
                    <a
                      href={`https://wa.me/923112233379?text=${encodeURIComponent(`Hi, I'm interested in ${project.name} at Etihad Town.`)}`}
